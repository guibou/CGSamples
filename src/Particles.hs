...
displayModel ps = translate (-fromIntegral (w `div` 2)) (-fromIntegral (h `div` 2)) $ Pictures (color black (Polygon [(0, 0), (fromIntegral w, 0), (fromIntegral w, fromIntegral h), (0, fromIntegral h)]) : map displayParticle ps)

displayParticle (Particle (V2 x y) _ _) = translate (x * fromIntegral w) (y * fromIntegral h) (color red (circleSolid 2))
...
advanceParticle :: Float -> Particle -> Particle
advanceParticle dt p = p {oldPosition = position p,
...
densityRelaxation dt radius ps = zipWith (relax dt) ps (particlesDensity radius ps)

particlesDensity :: Float -> [Particle] -> [[(Float, V2)]]
particlesDensity radius ps = map (dFriends radius densityGrid) ps
  where
    densityGrid = computeDensityGrid radius ps

dFriends :: Float -> Grid -> Particle -> [(Float, V2)]
dFriends radius grid p = do
          let (x0, y0) = particleHash radius p
          x <- [x0 - 1..x0 + 1]
          y <- [y0 - 1..y0 + 1]
...
computeDensityGrid radius ps = HashMap.fromListWith (\[x] xs -> x:xs) (map (\p -> (particleHash radius p, [p])) ps)

particleHash :: Float -> Particle -> (Int, Int)
particleHash radius p = (truncate (x / radius), truncate (y / radius))
  where
    V2 x y = position p
